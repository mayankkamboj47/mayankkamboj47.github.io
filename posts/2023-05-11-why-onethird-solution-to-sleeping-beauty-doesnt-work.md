...
> ### Abstract
> This paper shows that Elga's P(H)=1/3 solution for Sleeping Beauty is wrong. It shows that Elga uses 3 basic assumptions to arrive at his answer, which can't hold together. It also discusses the problem with frequentist reasoning which claims that guessing tails allows one to be correct more often on average. Finally, formalises the problem such that heads and tails are both equally likely on a Monday, and yet P(H) remains 1/2. 

Due to various points of contention, combined with an incorrect solution proposed in the original paper, the Sleeping Beauty problem has created a lot of confusion. In this document, I aim to provide clarity and settle some of the ongoing debate.
...
In this case, it's the third premise. First, and the weakest argument I'll make against it, is that it doesn't matter whether the coin is tossed on Monday or Sunday *only if Beauty doesn't know about it*. If she does, Monday carries additional information for her that it didn't before. Elga's justification contains no consideration for this possibility. He merely says : 
